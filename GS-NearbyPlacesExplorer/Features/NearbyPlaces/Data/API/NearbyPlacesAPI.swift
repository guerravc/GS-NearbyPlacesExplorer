...
/// API endpoints for the NearbyPlaces module.
///
...
public enum NearbyPlacesAPI {
  /// Fetches data for the given request.
  case fetch(request: NearbyPlacesRequest)
  /// Persists the given entity DTO (create/update).
  case persist(dto: NearbyPlacesEntityDTO)
}

extension NearbyPlacesAPI: APIRouter {
  ...
  public var path: String {
    switch self {
    case .fetch:
      return "/NearbyPlaces/fetch"
    case .persist:
      return "/NearbyPlaces/persist"
    }
  }
  ...
  public var body: Data? {
    ...
    case let .persist(dto):
      do {
        ...
        assertionFailure("Failed to encode NearbyPlacesEntityDTO body: \(error)")
        return nil
      }
    }
  }
