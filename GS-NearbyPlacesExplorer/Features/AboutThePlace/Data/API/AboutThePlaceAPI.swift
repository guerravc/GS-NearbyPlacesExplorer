...
/// API endpoints for the AboutThePlace module.
///
...
public enum AboutThePlaceAPI {
  /// Fetches data for the given request.
  case fetch(request: AboutThePlaceRequest)
  /// Persists the given entity DTO (create/update).
  case persist(dto: AboutThePlaceEntityDTO)
}

extension AboutThePlaceAPI: APIRouter {
  ...
  public var path: String {
    switch self {
    case .fetch:
      return "/AboutThePlace/fetch"
    case .persist:
      return "/AboutThePlace/persist"
    }
  }
  ...
  public var body: Data? {
    ...
    case let .persist(dto):
      do {
        ...
        assertionFailure("Failed to encode AboutThePlaceEntityDTO body: \(error)")
        return nil
      }
    }
  }
