...
/// Default remote service implementation for the AboutThePlace module.
///
/// This type implements `AboutThePlaceRemoteDataSource` using the
/// Networking Core (`APIClient`) and the module-specific `AboutThePlaceAPI`
/// router. It is responsible for performing HTTP calls and decoding responses
/// into DTOs wrapped by `BaseResponse` envelopes.
public final class DefaultAboutThePlaceService: AboutThePlaceRemoteDataSource {
  ...
  // MARK: - AboutThePlaceRemoteDataSource
  ...
  ///   "status": 0,
  ///   "message": "OK",
  ///   "object": { /* AboutThePlaceResponseDTO fields */ }
  /// }
  /// ```
  ///
  ...
  ) async throws -> BaseResponse<AboutThePlaceResponseDTO> {
    let route = AboutThePlaceAPI.fetch(request: request)

    let result: Result<BaseResponse<AboutThePlaceResponseDTO>, NetworkError> =
  ...
  /// - Throws: A `NetworkError` if the request fails or the payload cannot be decoded.
  public func persist(
    _ dto: AboutThePlaceEntityDTO
  ) async throws -> EmptyBaseResponse {
    let route = AboutThePlaceAPI.persist(dto: dto)
